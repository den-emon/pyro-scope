<?php
/*
Plugin Name: Pyre Scope
Description: Pyre Shield専用補完プラグイン（スキャン・脆弱性・整合性チェック）
Version: 3.2
Author: Ikkido (一揆堂無門)
Requires PHP: 8.1
...
register_activation_hook(__FILE__, static function (): void {
    if (!wp_next_scheduled('pyre_scope_weekly_scan_event')) {
        wp_schedule_event(time(), 'weekly', 'pyre_scope_weekly_scan_event');
    }
});

register_deactivation_hook(__FILE__, static function (): void {
    wp_clear_scheduled_hook('pyre_scope_weekly_scan_event');
});

final class Pyre_Scope
{
    // [H2] PHP 8.1準拠の型宣言を全プロパティに追加
    private static ?self $instance = null;
    private const PLUGIN_VERSION = '3.2';
    private string $option_key = 'pyre_scope_options';
    ...
    private function get_log_file_path(): string
    {
        ...
        $log_dir = $upload_dir['basedir'] . '/fspo-log';
        ...
        $this->log_file_path = $log_dir . '/fspo-scan.json';
    ...
    private function load_options(): void
    {
        $defaults = [
            ...
            'enable_integrity' => 1,
            'enable_vuln'      => 1,
            'whitelist_paths'  => ['wp-content/plugins/pyre-shield'],
        ];
    ...
    private function add_hooks(): void
    {
        ...
        add_action('wp_ajax_pyre_scope_run_scan', [$this, 'ajax_run_scan']);
        add_action('pyre_scope_weekly_scan_event', [$this, 'run_scheduled_scan']);
    }
    ...
    {
        if ('toplevel_page_pyre-scope' !== $hook) {
            return;
        }
        wp_enqueue_style(
            'pyre-scope-admin-style',
            plugin_dir_url(__FILE__) . 'assets/css/pyre-scope-admin.css',
            [],
            self::PLUGIN_VERSION
        );
        wp_enqueue_script(
            'pyre-scope-admin-script',
            plugin_dir_url(__FILE__) . 'assets/js/pyre-scope-admin.js',
            ['jquery'],
            self::PLUGIN_VERSION,
            true
        );
        wp_localize_script(
            ...
            [
                ...
                'nonce'    => wp_create_nonce('pyre_scope_scan_nonce'),
            ]
        );
    }
    ...
    public function settings_page(): void
    {
        ...
        <div class="wrap">
            ...
            <p style="font-size:1.1em; background:#fff; padding:10px; border-left: 4px solid #72aee6;">
                <?php
                $last_scan_timestamp = get_option('pyre_scope_last_scan_timestamp');
            ...
            <button id="pyre-scope-run-scan" class="button button-primary">Run manual scan now</button>
            <div id="pyre-scope-progress-container" style="margin-top:10px; width:100%; background:#eee; height:20px; border-radius:3px; display:none;">
                <div id="pyre-scope-progress-bar" style="width:0%; height:100%; background:#0a0; transition: width 0.1s;"></div>
            </div>
            <pre id="pyre-scope-scan-log" style="margin-top:10px; background:#f5f5f5; padding:10px; height:200px; overflow:auto; border:1px solid #ccc;"></pre>

            <div id="pyre-scope-results-container">
                <?php
    ...
    {
        check_ajax_referer('pyre_scope_scan_nonce');
    ...
    private function execute_scan(): array
    {
        ...
        update_option('pyre_scope_last_scan_timestamp', $scan_timestamp, false);
    ...
    private function generate_results_html(array $results): string
    {
        ...
        if (is_string($first_issue) && str_starts_with($first_issue, 'Error:')) {
            ...
            $html .= '<div class="pyre-scope-notice info">';
        ...
        if (!empty($files)) {
            ...
            $html .= '<table class="pyre-scope-results-table">';
    ...
    private function check_vuln(): array
    {
        ...
        foreach ($all_plugins as $path => $meta) {
            ...
            $cache_key = 'pyre_scope_vuln_' . md5($slug);
