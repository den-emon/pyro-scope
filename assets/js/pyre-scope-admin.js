jQuery(document).ready(function($) {
    ...
    // [FIXED] セレクタ・ローカライズオブジェクト名・AJAXアクション名を pyre-scope / PyreScopeAjax に変更
    // スキャンボタンがクリックされたときの処理
    $('#pyre-scope-run-scan').on('click', function() {
        // 各UI要素を定数として定義
        const scanButton = $(this);
        const log = $('#pyre-scope-scan-log');
        const progressBar = $('#pyre-scope-progress-bar');
        const progressContainer = $('#pyre-scope-progress-container');

        // --- スキャン開始時のUI初期化 ---
        scanButton.prop('disabled', true);
        log.text('Scan initialized...');
        $('#pyre-scope-results-container').html(''); // 前回のリザルトをクリア
        ...
        $.ajax({
            ...
            url: PyreScopeAjax.ajax_url,
            type: 'POST',
            dataType: 'json',
            data: {
                action: 'pyre_scope_run_scan',
                _ajax_nonce: PyreScopeAjax.nonce // セキュリティトークン
            },
            success: function(resp) {
                if (resp.success) {
                    // 成功した場合、レスポンス内のHTMLを結果表示エリアに描画
                    $('#pyre-scope-results-container').html(resp.data.html);
